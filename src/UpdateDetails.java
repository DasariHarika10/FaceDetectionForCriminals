	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (ae.getSource() == b2) {
			try {
				...
				String str = "Update face set fname='" + fname + "',lname='" + lname + "',aname='" + aname
						+ "',dob=to_date('" + dob + "-" + mob + "-" + yob + "','DD-Month-YYYY'), age= " + age
						+ ",gender='" + gender + "',address='" + address + "',city='" + city + "',state='" + state
						+ "',crimein='" + crime + "',photo='" + photofile + "' where cid=" + cid;
				...
				JOptionPane.showMessageDialog(this, e.getMessage(), "Updating the Criminal Details",
			...
			t10.setText("");
			t11.setText("");
			photo.setIcon(new ImageIcon(getClass().getResource("Icons\\anand.bmp")));
			...
			ImageIcon ico = new ImageIcon(getClass().getResource("Images\\" + photofile));
			photo.setIcon(ico);
			...
			if (idc.getSelectedIndex() != 0) {
				try {
					...
					ResultSet rs = stmt.executeQuery(
							"select cid, fname, lname, aname, to_char(dob,'DD'), to_char(dob,'MM'), to_char(dob,'YYYY'), age, gender, address, city, state, arresteddate, crimein, photo from face where cid="
									+ cid);
					if (rs.next()) {
						b4.setEnabled(true);
						...
						ImageIcon icophoto = new ImageIcon(getClass().getResource("Images\\" + photofile));

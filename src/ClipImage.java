...
class ClipImageAnand extends JFrame implements ActionListener {
	...
	ClipImageAnand() {
		...
		try {
			...
			cn = DriverManager.getConnection(d1.host, d1.username, d1.password);
	...
	public void actionPerformed(ActionEvent evt) {
		if (evt.getSource() == btnPhoto) {
			...
			if (selcrimid.equalsIgnoreCase("Select Criminal"))
				JOptionPane.showMessageDialog(this, "Select the ID of the Criminal to clip the Photo",
						"clipping Photograph", JOptionPane.ERROR_MESSAGE);
			else {
				imgstatus = true;
				try {
					...
					if (rs.next()) {
						...
						JOptionPane.showMessageDialog(this, "Criminal Selected", "clipping Photograph",
		...
		if (evt.getSource() == btnHair) {
			...
			try {
				FileOutputStream fout = new FileOutputStream(getClass().getResource("Clips\\hair\\hair" + selcrimid + ".gif").getFile());
		...
		if (evt.getSource() == btnEyes) {
			...
			try {
				FileOutputStream fout = new FileOutputStream(getClass().getResource("Clips\\eyes\\eyes" + selcrimid + ".gif").getFile());
				DataOutputStream dout = new DataOutputStream(fout);
				for (int i = 0; i < pixels.length; i++) {
					dout.writeInt(pixels[i]);
				}
		...
		if (evt.getSource() == btnForeHead) {
			...
			try {
				FileOutputStream fout = new FileOutputStream(getClass().getResource("Clips\\forehead\\forehead" + selcrimid + ".gif").getFile());
				DataOutputStream dout = new DataOutputStream(fout);
				for (int i = 0; i < pixels.length; i++) {
					dout.writeInt(pixels[i]);
				}
		...
		if (evt.getSource() == btnNose) {
			...
			try {
				FileOutputStream fout = new FileOutputStream(getClass().getResource("Clips\\nose\\nose" + selcrimid + ".gif").getFile());
				DataOutputStream dout = new DataOutputStream(fout);
				for (int i = 0; i < pixels.length; i++) {
					dout.writeInt(pixels[i]);
				}
		...
		if (evt.getSource() == btnChin) {
			...
			try {
				FileOutputStream fout = new FileOutputStream(getClass().getResource("Clips\\chin\\chin" + selcrimid + ".gif").getFile());
				DataOutputStream dout = new DataOutputStream(fout);
				for (int i = 0; i < pixels.length; i++) {
					dout.writeInt(pixels[i]);
				}
		...
		if (evt.getSource() == btnSave) {
			try {
				String str = "insert into face_height(cid, hair, forehead, eyes, nose, lips, chin) values(" + selcrimid
						+ "," + htHair + "," + htForeHead + "," + htEyes + "," + htNose + "," + htLips + "," + htChin
						+ ")";
				stmt.executeUpdate(str);
				JOptionPane.showMessageDialog(this, "Clips Stored Successfully", "Clipping Images",
						JOptionPane.INFORMATION_MESSAGE);
			} catch (Exception e) {
			}
		}
		...
		if (evt.getSource() == clearHair) {
			lblHair.setIcon(new ImageIcon(getClass().getResource("Icons\\anand.bmp")));
		}
		if (evt.getSource() == clearForeHead) {
			lblForeHead.setIcon(new ImageIcon(getClass().getResource("Icons\\anand.bmp")));
		}
		if (evt.getSource() == clearEyes) {
			lblEyes.setIcon(new ImageIcon(getClass().getResource("Icons\\anand.bmp")));
		}
		if (evt.getSource() == clearNose) {
			lblNose.setIcon(new ImageIcon(getClass().getResource("Icons\\anand.bmp")));
		}
		if (evt.getSource() == clearLips) {
			lblLips.setIcon(new ImageIcon(getClass().getResource("Icons\\anand.bmp")));
		}
		if (evt.getSource() == clearChin) {
			lblChin.setIcon(new ImageIcon(getClass().getResource("Icons\\anand.bmp")));
		}
	...
	class SourceImageCanvas extends Canvas implements MouseListener {
		...
		public void paint(Graphics g) {
			if (imgstatus) {
				orgimg = tkt.getImage(getClass().getResource("Images\\" + imgfile));
